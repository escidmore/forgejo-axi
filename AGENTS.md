...
- `src/help.ts` is the single source of command guidance: the CLI serves it on `--help` and `src/skill.ts` renders `skills/forgejo-axi/SKILL.md` from it. Never hand-edit the skill; change `src/help.ts` and run `npm run gen:skill`. `npm run check` fails on drift.
