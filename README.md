...
Installing the skill installs guidance only; it does not install the CLI runtime, which comes from the source install above. The file is generated by `npm run gen:skill` from `src/help.ts` — the same help text the CLI serves on `--help` — and `npm run check` fails when the committed file drifts from it.
