...
No npm package or release is published yet, so the CLI cannot be fetched with a registry runner.
...
Installing the skill installs guidance only; it does not install the CLI runtime, which comes from the source install above. The file is generated by `npm run gen:skill`: its command catalog comes from `src/help.ts`, the same help text the CLI serves on `--help`, and the guidance around that catalog from `src/skill.ts`. `npm run check` fails when the committed file drifts from either.
