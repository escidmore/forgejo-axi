...
A run identity is `{id, url, api_url, title, event, branch, head_sha, run_number, status, started_at, completed_at}`. `url` and `api_url` are constructed from the configured canonical base URL and repository identity, not trusted response links. `branch` is Forgejo's head branch and `status` is the run state Forgejo reports, defaulting to `unknown`; `started_at` and `completed_at` are `null` while unset. A job identity is `{id, run_id, name, status, started_at, completed_at}`, plus `log` when a log was requested and folded in.
...
`run download` writes each artifact to `{--dir}/{name}.zip`, creating `--dir` and its parents when missing, and narrows to one artifact with `--name`. An existing file is never overwritten: the download fails with `ARTIFACT_EXISTS` (exit `1`) naming the path. An artifact name Forgejo returns that would escape the directory is refused as `INVALID_RESPONSE`.
