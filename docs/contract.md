...
Authentication resolves `--token-env` first, then `FORGEJO_TOKEN_<HOST_KEY>`, then `FORGEJO_TOKEN` when the base URL came from `FORGEJO_BASE_URL` or the hosts file, then the matching hosts-file entry. An explicitly named `--token-env` variable that is unset or empty is a usage error. Tokens are never accepted as argument values or emitted. The CLI reads but never writes the hosts file.
