...
Capabilities are runtime-probed booleans, never version assumptions. If the Swagger document is unavailable, forbidden, rate-limited, malformed, times out, or returns a server error after the API version probe succeeds, status returns all capability booleans false with `probe.complete=false` instead of failing the entire status command. Forgejo 15.0.5 advertises the Actions run list and detail routes but not the run-jobs or job-log routes; Forgejo 16 reports each capability only when its runtime document advertises the corresponding route. Unsupported logs never alter commit-status semantics.
