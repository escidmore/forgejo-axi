...
`review_decision` is `changes_requested|approved|stale|review_requested|commented|none`. Only each reviewer's latest non-dismissed approval or change request is decisive; change requests from different reviewers outrank approvals. A current verdict attached only to an older commit is `stale`. If review pagination is incomplete, the field is `null` and `field_info.failures` names `review_decision` rather than reporting a decision from partial history.
