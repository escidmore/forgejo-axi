...
Each lane also drives content history, the one family served from Forgejo's web root rather than `/api/v1`, so no other probe covers its auth or its response shape. That web root authenticates by session and never by API token, so it cannot read a private repository however well the token reads the API, and it answers for one with the same 404 as a host too old to serve the routes at all. The lane edits an issue body and then follows whichever of those two worlds it is in. Where the repository is readable, it asserts the overview counts the body's revisions, that `list` returns the original alongside the edit newest first, that `detail` reconstructs a `before` holding only the old text and an `after` holding only the new one, and that the `issue view` enrichment agrees with the overview; soft-delete then follows the host's verdict — granted, a revision is deleted and the repeat is an already-deleted no-op; withheld, the CLI must refuse with `CONTENT_HISTORY_DELETE_REFUSED` before posting anything. Where it is not — the private repository both lanes currently run against — the lane asserts the CLI names that as `CONTENT_HISTORY_AUTHORIZATION` on both `overview` and `list`, never as an unsupported host, a claim the same host disproves for any public repository; and that `issue view` still succeeds with the enrichment simply absent.
...
Not yet covered live, each needing more than a disposable repository: the `CONFLICT` race-recovery branch in `pr create`, which needs a competing create landed inside one invocation's pre-check-to-`POST` window; content history's `detail` and `soft-delete`, which need a repository the web root can read and so a public one, leaving the reconstruction and the deletion permission check fixture-only; reviews from a second account, so the `APPROVED` and `REQUEST_CHANGES` verdicts and the stale and dismissed flags stay fixture-only; and transport behaviour — path prefixes, redirects, and CA files, which the CI workflow wires up for a host behind a private CA but which no run has yet exercised.
