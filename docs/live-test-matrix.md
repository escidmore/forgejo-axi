...
Each lane also drives content history, the one family served from Forgejo's web root rather than `/api/v1`, so no other probe covers its auth or its response shape. The lane edits an issue body, then asserts the overview counts the body's revisions, that `list` returns the original alongside the edit newest first, and that `detail` reconstructs a `before` holding only the old text and an `after` holding only the new one. It then reads `issue view` and asserts the enrichment agrees with the overview — reached only if the view succeeded at all, which is the assertion that matters when the history routes refuse a token they may never have accepted. Soft-delete follows whichever verdict the host gives: granted, a revision is deleted and the repeat is an already-deleted no-op; withheld, the CLI must refuse with `CONTENT_HISTORY_DELETE_REFUSED` before posting anything.
